...
public class Fragement_ProductDetailView extends Fragment implements
        ...
        f_tradify.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (UserContext.USERID.equals(user.getUserId())) {
                    ...
                    getRequest(getActivity().getApplicationContext());
    ...
    public static void getRequest(Context context) {
        try {
            ...
            httpClient.addHeader("X-Authorization", "7f724f18cdd8e52e544ce2eaefedaf26");
            String url = "https://api.batch.com/1.0/DEV5724364173DA8BE521A1E592F18/transactional/send";
            JSONObject json = new JSONObject();
            json.put("group_id", "welcome");
            JSONObject jsonTok = new JSONObject();
            JSONArray array = new JSONArray();
            array.put("APA91bF_iFyo5EoUZ8vXkIJ9w2z-u9S5ivh_-qgsWzsmH4bfEUTKy3PyFK8C4w19ylFRPAsv-qvtfhf6qocytECb6Ec7O0X7C62u37v4TFZurG_zIrTd6MSLlcbyMjq8vFaBwswgq2O_");
            jsonTok.put("tokens",array);

            //Uncomment to test specific user notification
            /*JSONArray userArray = new JSONArray();
            userArray.put(user.getUserId());
            jsonTok.put("custom_ids",userArray);*/

            json.put("recipients",jsonTok);
            JSONObject jsonTitle = new JSONObject();
            jsonTitle.put("title","Hurry Up! "+product.getProductName()+" trending HOT!");
            jsonTitle.put("body","Tradify "+product.getProductName()+" as soon as possible. Many People are interested in it.");
            json.put("message",jsonTitle);
            //JSONObject jsonCusPayload = new JSONObject();
            //json.put("custom_payload","{\"tag\":\"wake up push\", \"landing_screen\":\"greeting\"}");
            Log.d("JSON INPUT: ", json.toString());
            //String jsonParamsString = "{\"group_id\":\"welcome\",\"recipients\":{\"tokens\":[\"APA91bF_iFyo5EoUZ8vXkIJ9w2z-u9S5ivh_-qgsWzsmH4bfEUTKy3PyFK8C4w19ylFRPAsv-qvtfhf6qocytECb6Ec7O0X7C62u37v4TFZurG_zIrTd6MSLlcbyMjq8vFaBwswgq2O_\"]},\"message\":{\"title\":\"Hello!\",\"body\":\"How's it going?\"}}";
            StringEntity entity = new StringEntity(json.toString());
            //RequestParams requestParams = new RequestParams(json);
            httpClient.post(context,url,entity,"application/json",new JsonHttpResponseHandler() {
                @Override
                ...
                public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                    super.onSuccess(statusCode, headers, response);
                    Log.d("Success in json", "Success in json");
                }
            ...
            /*httpClient.post(url, requestParams, new JsonHttpResponseHandler() {
                @Override
                public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                    super.onSuccess(statusCode, headers, response);
                    Log.d("Success", "Success");
